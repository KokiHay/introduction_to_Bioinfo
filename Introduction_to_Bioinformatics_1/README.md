...
### 3. MiniCondaのインストール  
パッケージマネージャをインストールしましょう。Minicondaを使うことで簡単にいろいろな解析ツールを利用できます。目的のツールに必要な関連ツールも同時にダウンロード・バージョン管理してくれるからです。MinicondaはAnacondaのコンパクト版です。Anacondaはpythonを用いたプログラミングではもっとも人気のあるディストリビューションですが、多様な言語に対応しています。baseとは別にpython2等の仮想環境を構築するのも簡単です。

[Miniconda](https://docs.conda.io/en/latest/miniconda.html)をインストールしましょう。自分のコンピュータのOSにあったインストーラをダウンロードし、インストーラの指示にしたがってインストールしましょう。インストールが終わったら、terminalを開いて、
...
インストーラがPATHを通してくれますので、minicondaに含まれているパッケージ (インストールしたパッケージを含む)はどこからでもパスなしのコマンドで呼び出すことができます。
...
bashコマンド (Linuxのコマンドラインで打ち込むコマンド)に慣れるためにも少しだけ手を動かしてパスの感覚を掴みましょう。bashではスペースの有無が重要ですので気をつけましょう。
...
~~~bash
  ...
  $ which conda # condaコマンドの実行ファイルがどこにあるか表示します
~~~

少しずつ慣れますから心配しないでください。キーボードのショートカットをも覚えると便利です。便利なものを以下に挙げます。他にもたくさんありますので、調べてみてください。  
- tabを押すことで、パスが補完されます。  
...
### 5. ATOMをインストールする
ATOMは2015年にリリースされた新しいテキストエディタです。テキストエディタは皆さんがコードを書くためのアプリです。選択肢はたくさんありますが、[ATOM](https://atom.io)がいいのではないかと思います。ダウンロードしてインストールしてください。インストールしたら、ATOMがアプリケーションのディレクトリに入っていることを確認してください。  
Rscriptを書くときにはRstudioでよいと思いますが、その他の言語はATOMがよいかもしれません。(pythonには大人気のjupyter notebookという対話型実行環境がありますが) ATOMは、有名なソフトウェア開発プラットホームでソースコードのリポジトリでもある[GitHub](https://github.com)にサポートされていて、多くのPackageを利用することができるため、簡単に機能を拡張できます。  
使い方は演習を通して説明します。

### 6. 解析ソフト(blast)を使ってみる
解析ツールはAnacondaのリポジトリからインストールするのがもっとも簡単です。したがって、使いたいツールがある場合にはまず、[Anaconda Cloud](https://anaconda.org/anaconda/repo)を探しましょう。解析ツールを使用する前には、できるだけ**Documentationに目を通しておく**ようにしましょう。  
...
今回は例として、ニチニチソウ ([Franke, Jakob et al. (2018), Data from: Gene discovery in Gelsemium highlights conserved gene clusters in monoterpene indole alkaloid biosynthesis, Dryad, Dataset](https://datadryad.org/stash/dataset/doi:10.5061/dryad.08vv50n))を用います。後ほど使うゲノムのfastaファイルとgffファイルも一緒にダウンロードします。コマンドラインからダウンロードする必要はありませんが、コマンドラインからダウンロードするコードを以下に書きました。websiteからダウンロードして、~Desktop/bioinfo_introに入れていただいてもかまいません。（この方が簡単ですね）
...
~~~
これで、~Desktop/bioinfo_introに２つのfastaファイルと１つのgffファイルが入ったと思います。fastaファイルは生物学ではよくお目にかかります。ただのテキストファイルですが、必ず各要素が">"の記号で始まります。その後ろに、要素のid、スペース、コメントが続き、改行した後に、文字列が並んでいます。fastaファイルはDNA配列やアミノ酸配列を記述するのに用いられます。拡張子は.fastaや.faなどです。少し中を覗いてみましょう。
...
~~~bash
  ...
  　# extract all three
  $ seqkit faidx cro_proteins.fasta CRO_T109448 CRO_T125328 CRO_T125329 -f　# -f full name
  $ seqkit faidx cro_proteins.fasta \
    CRO_T109448 CRO_T125328 CRO_T125329 -f > ./blast_tutorial/test.fa
  　# > は左の中身を右のファイルに格納するという意味です。
~~~
...
第一回はこれで終わります。今回は皆さんにとって有用であろう種々のアプリケーションのインストールとcondaでinstallしたpackage (blast)の使い方を説明しました。
第二回では、今回インストールしたRstudio、Atomの使い方と共に、より実践的なプログラミングに触れようと思います。
